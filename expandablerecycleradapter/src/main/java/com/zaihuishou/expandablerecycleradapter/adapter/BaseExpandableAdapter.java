...
public abstract class BaseExpandableAdapter extends RecyclerView.Adapter implements AbstractExpandableAdapterItem.ParentListItemExpandCollapseListener {
    ...
    private ExpandableListItem lastExpandItem = null;
    ...
    @Override
    public void onParentListItemCollapsed(ExpandableListItem item, int position) {
        Object o = mDataList.get(position);
     ...
     */
    @Override
    public void onParentListItemExpanded(ExpandableListItem item, int position) {
        try {
            if (lastExpandItem != null) {
                int lastIndex = mDataList.indexOf(lastExpandItem);
                Log.d("yanglin", "收起的position" + lastIndex);
                collapseParentListItem(lastExpandItem, lastIndex, false);
            }
            position = mDataList.indexOf(item);
            Log.d("yanglin", "展开的position" + position);
            expandParentListItem(item, position, true, false);
            lastExpandItem = item;
    ...
    @Override
    public void onParentListItemOnlyClick(ExpandableListItem item, int position) {
        try {
            Object o = mDataList.get(position);
            if (o instanceof ExpandableListItem) {
                onlyCilckParentListItem(item);
            }
        } catch (IndexOutOfBoundsException e) {
            Log.d("yanglin", "异常" + e.getMessage());
        }
    }

    private void onlyCilckParentListItem(ExpandableListItem item) {
        if (mExpandCollapseListener != null) {
            mExpandCollapseListener.onChildItemClick(item);
        }
    }
...
//    }
    ...
    public interface ExpandCollapseListener {
         ...
         * 当前点击对应的tiem对象
         */
        void onChildItemClick(ExpandableListItem item);
    }
}
