...
//        compatibilityDataSizeChanged(mData.size());
    }

    public void addDataNotifyAll(@IntRange(from = 0) int position, @NonNull T data) {
        if (position > mData.size())
            return;
        mData.add(position, data);
        position = position + getHeaderLayoutCount();
    ...
    public void remove(@IntRange(from = 0) int position) {
        ...
        mData.remove(internalPosition);
        notifyItemRemoved(internalPosition);
        notifyItemRangeChanged(internalPosition, mData.size());
        compatibilityDataSizeChanged(mData.size());
    }

    public void removeHeaderItem(@IntRange(from = 0) int position, @NonNull T data) {
        notifyDataSetChanged();
    }
    ...
    public void setData(@IntRange(from = 0) int index, @NonNull T data) {
        ...
        notifyItemChanged(mData.indexOf(data) + getHeaderLayoutCount());
    }

    /**
     * 更新指定位置data，并且移动到正确顺序
     */
    public void setDataAndRotate(@IntRange(from = 0) int oldIndex, @IntRange(from = 0) int newIndex, @NonNull T data, int empytCount) {
        //原位置的data先更新
        if (oldIndex != newIndex) {
            mData.set(oldIndex, data);
            List<T> list = mData.subList(newIndex, oldIndex + 1);
            Collections.rotate(list, 1);
            notifyDataSetChanged();
        } else {
            setData(oldIndex, data);
        }
    }
