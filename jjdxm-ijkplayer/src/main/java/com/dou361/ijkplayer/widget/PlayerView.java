...
import com.dou361.ijkplayer.utils.AnimationUtils;
...
//    private final ImageView iv_rotation;
     ...
     * 视频bottonbar的播放按钮区域
     */
    private final LinearLayout iv_bar_player_ly;
...
//    private final TextView tv_steam;
     ...
     * 显示背景音乐按钮
     */
//    private final ImageView showMenuImg;

    private View showMenuLy;
    /**
     * 背景音乐框
     */
    private MPopWindow menuWindow;

    /**
     * 当前选择的背景音乐id
     */
    private int mMusicItem = 5;
    /**
     * 背景音乐播放器
     */
...
//    private final StreamSelectAdapter streamSelectAdapter;
    ...
    private voiceItemOnClickListener itemOnClickL;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MESSAGE_HIDE_CENTER_BOX:
                    AnimationUtils.performHideViewAnim(mContext, llCenterStatusLy, R.anim.alpha_out_anim);
                    break;
                /**滑动完成，设置播放进度*/
...
//                showStreamSelectView();
                ...
                /**视频全屏切换*/
                toggleFullScreen();
            } else if (v.getId() == ResourceUtils.getResourceIdByName(mContext, "id", "video_play_play_ly")) {
                if (mAutoPlayRunnable != null) mAutoPlayRunnable.resetStart();
                updatePausePlay(3);
                /**视频播放和暂停*/
            ...
            } else if (v.getId() == ResourceUtils.getResourceIdByName(mContext, "id", "video_play_back_img")) {
                /**返回,此处不需要竖屏，只要横屏*/
                mActivity.finish();
                ...
                /**重新播放*/
                isPlayOver = status == PlayStateParams.STATE_COMPLETED;
                status = PlayStateParams.STATE_ERROR;
                hideStatusUI(3);
                startPlay();
                updatePausePlay(5);
            } else if (v.getId() == ResourceUtils.getResourceIdByName(mContext, "id", "music_bg")) {
                //显示音乐菜单
                Log.d("yanglin", "音乐菜单点击1");
                if (itemOnClickL != null) {
                    Log.d("yanglin", "音乐菜单点击2");
                    itemOnClickL.itemOnClick(true);
                }
                showMusicMenu();
            } else if (v.getId() == R.id.music_classic) {
                /**经典*/
                mMusicItem = ResourceUtils.getResourceIdByName(mContext, "raw", "classic");
                initVoiceMedia(mMusicItem);
            } else if (v.getId() == R.id.music_light) {
                /**轻快*/
                mMusicItem = ResourceUtils.getResourceIdByName(mContext, "raw", "panor_relaxed");
                initVoiceMedia(mMusicItem);
            } else if (v.getId() == R.id.music_scenery) {
                /**风景*/
                mMusicItem = ResourceUtils.getResourceIdByName(mContext, "raw", "scenery");
                initVoiceMedia(mMusicItem);
            } else if (v.getId() == R.id.music_shock) {
                /**震撼*/
                mMusicItem = ResourceUtils.getResourceIdByName(mContext, "raw", "panor_shock");
                initVoiceMedia(mMusicItem);
            } else if (v.getId() == R.id.music_relieve) {
                /**舒缓*/
                mMusicItem = ResourceUtils.getResourceIdByName(mContext, "raw", "relieve");
                initVoiceMedia(mMusicItem);
            } else if (v.getId() == R.id.music_original) {
                /**原声*/
                mMusicItem = 5;
                initVoiceMedia(mMusicItem);
            }
    ...
    private final SeekBar.OnSeekBarChangeListener mSeekListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (!fromUser) {
                ...
                iv_time.setText(time + "/" + generateTime(videoView.getDuration()));
            }
        }
    ...
    public PlayerView(Activity activity) {
        ...
        rl_box = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_paly_ly"));
        videoView = (IjkVideoView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_view"));
//        volumeControllerContainer = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "simple_player_volume_controller_container"));
        /**声音进度*/
//        volumeController = (SeekBar) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "volume_seekbar"));
//        volumeController.setMax(100);
//     showDefDialog   volumeController.setOnSeekBarChangeListener(this.onVolumeControllerChangeListener);
        /**亮度进度*/
//        brightnessController = (SeekBar) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "brightness_seekbar"));
...
//        streamSelectListView = (ListView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "simple_player_select_streams_list"));
        ll_topbar = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_title_ly"));
        iv_title = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_title_tv"));
        ll_bottombar = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_bottom_ly"));
        llCenterStatusLy = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_ly"));
        iv_trumb = (ImageView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "iv_trumb"));
        iv_back = (ImageView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_back_img"));
        iv_menu = (ImageView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_menu"));
        iv_bar_player = (ImageView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_play_img"));
        iv_bar_player_ly = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_play_ly"));
        iv_time = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_video_time_tv"));
...
//        tv_steam = (TextView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_stream"));
        ...
        seekBar = (SeekBar) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_seekbar"));
        seekBar.setMax(1000);
//        showMenuImg = (ImageView) activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "music_img"));
//        showMenuLy = activity.findViewById(ResourceUtils.getResourceIdByName(mContext, "id", "music_bg"));
//        showMenuLy.setOnClickListener(onClickListener);
        seekBar.setOnSeekBarChangeListener(mSeekListener);
        iv_bar_player_ly.setOnClickListener(onClickListener);
...
//        }
    ...
    public PlayerView setMenuOnClickListener(voiceItemOnClickListener l) {
        this.itemOnClickL = l;
        return this;
    }
    ...
    public PlayerView setPlaySource(String url) {
        this.url = url;
        videoView.setUsingAndroidPlayer(!url.startsWith("http"));
...
//        }
    ...
    public PlayerView setTitle(String title) {
        if (title != null) {
            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_title_tv")).text(title);
        } else {
            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_title_tv")).visibility(View.GONE);
        }
        return this;
    }
...
//        ll_topbar.setVisibility(isHideTopBar ? View.GONE : View.VISIBLE);
     ...
     * 顶部状态栏隐藏动画
     */
    private void topBarHide(boolean isHide) {
        if (isHide) {
            if (ll_topbar.getVisibility() == View.VISIBLE)
                AnimationUtils.performHideViewAnim(mContext, ll_topbar, R.anim.exit_top_anim);
        } else {
            if (ll_topbar.getVisibility() == View.GONE)
                AnimationUtils.performShowViewAnim(mContext, ll_topbar, R.anim.enter_top_anim);
        }
    }
     ...
     * 底部状态栏隐藏动画
     */
    private void bottomBarHide(boolean isHide) {
        Log.d("yanglin", "隐藏与显示===>" + isHide);
        if (isHide) {
            if (mAutoPlayRunnable != null)
                mAutoPlayRunnable.stop();
            if (ll_bottombar.getVisibility() == View.VISIBLE)
                AnimationUtils.performHideViewAnim(mContext, ll_bottombar, R.anim.exit_bottom_anim);
        } else {
            if (ll_bottombar.getVisibility() == View.GONE)
                AnimationUtils.performShowViewAnim(mContext, ll_bottombar, R.anim.enter_bottom_anim);
        }
    }
    ...
    public PlayerView operatorPanl() {
        ...
        if (isShowControlPanl) {
            topBarHide(isHideTopBar);
            bottomBarHide(isHideBottonBar);
            if (isLive) {
...
//            updatePausePlay(6);
            ...
            if (isHideTopBar) {
                ...
                topBarHide(!isForbidHideControlPanl);
            }
            if (isHideBottonBar) {
                bottomBarHide(true);
            } else {
                bottomBarHide(!isForbidHideControlPanl);
            }
...
//            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_endTime_full")).gone();
     ...
     * 状态改变同步UI
     */
    private void statusChange(final int newStatus) {
        Log.d("yl", "播放状态----->" + newStatus);
        if (newStatus == PlayStateParams.STATE_COMPLETED) {
            mHandler.removeMessages(MESSAGE_SHOW_PROGRESS);
            seekBar.setProgress(1000);
            //这里未用到，导致空指针
//            if (query.id(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_currentTime")) != null)
//                query.id(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_currentTime")).text(generateTime(videoView.getDuration()));
...
//            hideAll();
            iv_bar_player.setImageResource(ResourceUtils.getResourceIdByName(mContext, "mipmap", "play_img_def"));
//            iv_player.setVisibility(View.VISIBLE);
//            showStatus("播放结束");
...
//            hideStatusUI(5);
            ...
            startPlay();
            iv_bar_player.setImageResource(ResourceUtils.getResourceIdByName(mContext, "mipmap", "stop_play_img_def"));
            /**视频缓冲结束后隐藏缩列图*/
...
//                    updateFullScreenButton();
     ...
     * 隐藏状态界面
     */
    private void hideStatusUI(int where) {
        Log.d("yl", "播放按钮隐藏5" + "----->" + where);
        topBarHide(true);
        bottomBarHide(true);
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "simple_player_settings_container")).gone();
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "simple_player_select_stream_container")).gone();
...
//        Log.e("yanglin", "缓冲条进度----");
        ...
        if (seekBar != null) {
            if (duration > 0) {
                float pos = 1000f * position / duration;
                seekBar.setProgress((int) pos);
            }
            int percent = videoView.getBufferPercentage();
            if (!url.contains("FLYDOWN")) {
                Log.d("yl", "缓冲区大小-----" + percent);
                seekBar.setSecondaryProgress(percent * 10);
            }
        }
//        Log.e("yanglin", "缓冲条进度----" + generateTime(position));
        iv_time.setText(generateTime(position) + "/" + generateTime(videoView.getDuration()));
        return position;
    }
...
//    }
    ...
    private void onVolumeSlide(float percent) {
        ...
        // 显示
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_ly")).visible();
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_img"))
                .image(i == 0 ? ResourceUtils.getResourceIdByName(mContext, "mipmap", "video_play_voice_img")
                        : ResourceUtils.getResourceIdByName(mContext, "mipmap", "video_play_voice_img"));
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_tvb")).text(s + "%");
//        volumeController.setProgress(s);
//        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_volume")).text(s).visible();
    ...
    private void onProgressSlide(float percent) {
        ...
        if (showDelta != 0) {
            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_ly")).visible();
            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_img"))
                    .image(showDelta > 0 ? ResourceUtils.getResourceIdByName(mContext, "mipmap", "video_play_forward_img")
                            : ResourceUtils.getResourceIdByName(mContext, "mipmap", "video_play_retreat_img"));
            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_tvb")).text(generateTime(newPosition) + "/" + generateTime(duration));
//        brightnessController.setProgress(((int) (lpa.screenBrightness * 100)));
...
//            query.id(ResourceUtils.getResourceIdByName(mContext, "id", "app_video_fastForward_all")).text(generateTime(duration));
    ...
    private void onBrightnessSlide(float percent) {
        ...
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_ly")).visible();
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_img"))
                .image(lpa.screenBrightness == 0 ? ResourceUtils.getResourceIdByName(mContext, "mipmap", "video_play_brightness_img")
                        : ResourceUtils.getResourceIdByName(mContext, "mipmap", "video_play_brightness_img"));
        query.id(ResourceUtils.getResourceIdByName(mContext, "id", "video_play_center_tvb")).text((int) (lpa.screenBrightness * 100) + "%");
//        brightnessController.setProgress(((int) (lpa.screenBrightness * 100)));
...
//                        /**进度设置*/
    ...
    private void showMusicMenu() {
        ...
        View popupView = null;
        popupView = LayoutInflater.from(mActivity).inflate(R.layout.muice_layout,
                null);
        popupView.measure(View.MeasureSpec.UNSPECIFIED, View.MeasureSpec.UNSPECIFIED);
        popupHeight = popupView.getMeasuredHeight();
        popupWidth = popupView.getMeasuredWidth();
        showMenuLy.getLocationOnScreen(location);
        menuWindow = new MPopWindow(mActivity, onClickListener);
//        menuWindow.showAtLocation(showMenuImg, Gravity.NO_GRAVITY, (location[0] + showMenuImg.getWidth() / 2) - popupWidth / 2,
//                location[1] - popupHeight - 10);
        if (mMusicItem == ResourceUtils.getResourceIdByName(mContext, "raw", "classic")) {
            menuWindow.muiceClassic.setChecked(true);
        } else if (mMusicItem == ResourceUtils.getResourceIdByName(mContext, "raw", "panor_relaxed")) {
            menuWindow.muiceLight.setChecked(true);
        } else if (mMusicItem == ResourceUtils.getResourceIdByName(mContext, "raw", "scenery")) {
            menuWindow.muiceScenery.setChecked(true);
        } else if (mMusicItem == ResourceUtils.getResourceIdByName(mContext, "raw", "panor_shock")) {
            menuWindow.muiceShock.setChecked(true);
        } else if (mMusicItem == ResourceUtils.getResourceIdByName(mContext, "raw", "relieve")) {
            menuWindow.muiceRelieve.setChecked(true);
        } else if (mMusicItem == 5) {
            menuWindow.muiceOriginal.setChecked(true);
        }
        mAutoPlayRunnable.start();
    }
     ...
     * 初始化音频播放
     */
    private void initVoiceMedia(int voice) {
        mAutoPlayRunnable.resetStart();
        try {
            if (mpy != null) {
                mpy.reset();
            ...
            if (voice != 5) {
                videoView.mMediaPlayer.setVolume(0f, 0f);
                mpy = MediaPlayer.create(mActivity, voice);//重新设置要播放的音频
                mpy.setLooping(true);
                if (videoView != null && videoView.isPlaying()) {
                    mpy.start();
                }
            } else {
                videoView.mMediaPlayer.setVolume(0.5f, 0.5f);
            }
        } catch (Exception e) {
    ...
    public interface voiceItemOnClickListener {
        void itemOnClick(boolean click);
    }
