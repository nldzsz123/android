...
public class SideIndexBar extends View {
    ...
    private void init(Context context) {
        ...
        mPaint.setColor(getResources().getColor(R.color.color_b8b8b8));

        mTouchedPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mTouchedPaint.setTextSize(24);
        mTouchedPaint.setColor(getResources().getColor(R.color.color_38bcff));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float moveTvTop = mItemHeight / 2 + (fm.bottom - fm.top) / 2 - fm.bottom + mItemHeight * mCurrentIndex + mTopMargin;
        float moveTvButtom = mItemHeight / 2 + (fm.bottom - fm.top) / 2 - fm.bottom + mItemHeight * (mIndexItems.size() - 1) + mTopMargin;
        if (mItemHeight * mCurrentIndex < mItemHeight * mIndexItems.size() / 2) {
            mOverlayTextView.setY(moveTvTop);
            mOverlayTextView.setBackgroundResource(R.mipmap.country_code_down_dialog);
        } else {
            mOverlayTextView.setBackgroundResource(R.mipmap.country_code_up_dialog);
            mOverlayTextView.setY(moveTvTop - mOverlayTextView.getHeight());

        }
//        if (moveTvTop >= mOverlayTextView.getHeight() / 2 && (moveTvButtom - moveTvTop) >= mOverlayTextView.getHeight() / 2) {
