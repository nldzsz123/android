package com.feipai.flypai.ui.view.swipemenuLayout;
...
import static com.feipai.flypai.ui.view.swipemenuLayout.State.CLOSE;
import static com.feipai.flypai.ui.view.swipemenuLayout.State.RIGHTOPEN;
...
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    private boolean mCanLeftSwipe = true;//是否可以左滑
    private boolean mCanRightSwipe = true;//是否可以右滑
    ...
    private OnSwipeMenuStatusListener mListener;
    ...
    public void addSwipeMenuStatusListener(OnSwipeMenuStatusListener listener) {
        ...
        try {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (attr == R.styleable.SwipeMenuLayout_leftMenuView) {
                    ...
                    mCanLeftSwipe = typedArray.getBoolean(R.styleable.SwipeMenuLayout_canLeftSwipe, false);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (getScrollX() < 0) {
                    if (!mCanRightSwipe || mLeftView == null) {
                        scrollTo(0, 0);
                        Log.i(TAG, ">>>>滑动菜单----->右边边没有" + mCanRightSwipe);
                    } else {//左滑
                    ...
                    if (!mCanLeftSwipe || mRightView == null) {
                        ...
                        Log.i(TAG, ">>>>一直向左滑动菜单----->" + getScrollX() + "||||" + (mRightView.getRight() - mContentView.getRight() - mContentViewLp.rightMargin));
                        if (getScrollX() > mRightView.getRight() - mContentView.getRight() - mContentViewLp.rightMargin) {
                            ...
                            if (mListener != null) {
                                mListener.onMoveDistenceScale(1f);
                            }
                        } else {
                            if (mListener != null) {
                                mListener.onMoveDistenceScale(((float) getScrollX()) / ((float) (mRightView.getRight() - mContentView.getRight() - mContentViewLp.rightMargin)));
                            }
                            Log.i(TAG, ">>>>一直向左滑动菜单----->直到关闭");
                        }
                    }
                }
                //当处于水平滑动时，禁止父类拦截
...
//                }
    ...
    private void handlerSwipeMenu(State result) {
        ...
        if (mListener != null) {
            mListener.onMenuStatusListener(result);
            mListener.onMoveDistenceScale(mStateCache == RIGHTOPEN ? 1f : 0f);
        }
        invalidate();
    }
    ...
    private State isShouldOpen(int scrollX) {
        if (!(mScaledTouchSlop < Math.abs(finalyDistanceX))) {
            //直到抬起，移动的距离不足以满足移动要求，被视为无移动。
//            mStateCache = null;//应该保持之前的状态无变化
            return mStateCache;
        }
...
//                mScroller.startScroll(mViewCache.getScrollX(), 0, -mViewCache.getScrollX(), 0);
                mViewCache = null;
                mStateCache = null;
                handlerSwipeMenu(mStateCache);
                if (mListener != null)
                    mListener.onMenuStatusListener(mStateCache);
            }
        }
    }
    ...
    public interface OnSwipeMenuStatusListener {

        void onMenuStatusListener(State state);

        /**
         * 右侧菜单移动的比例0-1,1为菜单完全显示，0为菜单完全关闭
         */
        void onMoveDistenceScale(float scale);
    }
