...
public class CenterPointSeekBar extends View {
    ...
    private float progressRadius = 45f;
    ...
    public interface OnSeekBarChangeListener {
        void onOnSeekBarValueChange(CenterPointSeekBar bar,
                                    double value);
    }
    ...
    public CenterPointSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CenterPointSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        this.thumbImage = ResourceUtils.getDrawabeToBitmap(R.mipmap.editor_seek_thumb, AutoUtils.getPercentWidthSize(79), AutoUtils.getPercentHeightSize(79));
        this.thumbPressedImage = ResourceUtils.getDrawabeToBitmap(R.mipmap.editor_seek_thumb,  AutoUtils.getPercentWidthSize(79), AutoUtils.getPercentHeightSize(79));
        ...
        this.lineHeight = a.getInt(R.styleable.StartPointSeekBar_linHeight, 0);
        this.progressDrawable = a.getDrawable(R.styleable.StartPointSeekBar_progressDrawable);
        this.backgroundDrawable = a.getDrawable(R.styleable.StartPointSeekBar_backgroundDrawable);
        ...
        thumbHalfHeight = 0.5f * thumbHeight;
        if (lineHeight == 0)
            lineHeight = (int) (0.9f * thumbHalfHeight);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec,
        ...
        int width = AutoUtils.getPercentWidthSize(200);
    ...
    public void setProgress(double value) {
        ...
        if (newThumbValue > absoluteMaxValue || newThumbValue < absoluteMinValue) {
            LogUtils.d("设置progress=》" + value + "|||absoluteMaxValue=>" + absoluteMaxValue + "|||absoluteMinValue=>" + absoluteMinValue + "|||newThumbValue=>" + newThumbValue);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (backgroundDrawable != null) {
            backgroundDrawable.setBounds(Math.round(rect.left), Math.round(rect.top), Math.round(rect.right), Math.round(rect.bottom));
            backgroundDrawable.draw(canvas);
        } else {
            paint.setColor(defaultBackgroundColor);
            canvas.drawRoundRect(rect, progressRadius, progressRadius, paint);
        }
       ...
       /* paint.setColor(defaultRangeColor);
        ...
        if (progressDrawable != null) {
            progressDrawable.setBounds(Math.round(rect.left), Math.round(rect.top), Math.round(rect.right), Math.round(rect.bottom));
