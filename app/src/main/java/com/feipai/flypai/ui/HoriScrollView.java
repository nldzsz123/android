...
public class HoriScrollView extends ScrollPickerView<CharSequence> {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mMinTextSize = AutoUtils.getPercentWidthSize(typedArray.getDimensionPixelSize(
                    R.styleable.StringScrollPicker_spv_min_text_size, mMinTextSize));
            mMaxTextSize = AutoUtils.getPercentWidthSize(typedArray.getDimensionPixelSize(
            ...
            mMaxLineWidth = AutoUtils.getPercentWidthSize(typedArray.getDimensionPixelSize(R.styleable.StringScrollPicker_spv_max_line_width, mMaxLineWidth));
        ...
        if (relative == -1 || relative == 1) { // 上一个或下一个
            ...
            if ((relative == -1 && moveLength < 0)
                ...
                float rate = (itemSize - Math.abs(moveLength))
                        / itemSize;
                color = ViewUtils.computeGradientColor(mStartColor, mEndColor, rate);
            }
...
//     */
//    public void setData(List<String> strings, int sIndex) {
//        this.strings = strings;
//        if (sIndex < 0 || sIndex + 1 > strings.size()) {
//            selectIndex = strings.size() / 2;
//        } else {
//            selectIndex = sIndex;
//        }
//        invalidate();
...
//        if (strings.size() != 0) {
//            return strings.get(selectIndex);
//        }
//        return null;
//    }
}
