...
public class CustomLayout extends AutoRelativeLayout {
    //单行显示
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount == 2) {
            ...
            if (getChildAt(0) instanceof TextView) {
                tv = (EditText) getChildAt(0);
//                tv.setTextSize(AutoUtils.getPercentHeightSize(45));
                initTextParams(tv.getText(), tv.getMeasuredWidth(), tv.getPaint());
            } else {
                throw new RuntimeException("CustomLayout first child view not a EditText");
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (type == SINGLE_LINE || type == MULTI_LINE) {
            EditText tv = (EditText) getChildAt(0);
            View v1 = getChildAt(1);
            //设置第二个view在EditText文字末尾位置
