...
/**
 * Created by YangLin on 2018-06-02.
 */
...
public class CommandUtil {
     ...
     * @return 丢包率 如50%可得 50，注意：-1是默认值，返回-1表示获取失败
     */
    public static String getPacketLossFloat(String url, int count, int timeout) {
        String packetLossInfo = getPacketLoss(url, count, timeout);
//        MLog.log("ping回复==>" + packetLossInfo);
        return packetLossInfo;
//        if (packetLossInfo.indexOf("received, 0%") != -1) {
//            return 0;
...
//                = getDomain(url);
        ...
        String pingString = ping(createSimplePingCommand(count, timeout, domain));
//        LogUtils.d("ping结果1=" + pingString);
        if (null != pingString) {
            return pingString;
//            try {
//                String tempInfo = pingString.substring(pingString.indexOf("received,"));
////                MLog.log("ping结果2=" + tempInfo);
//                if (tempInfo.contains("errors")) {
//                    tempInfo = tempInfo.substring(tempInfo.indexOf("errors,"));
//                    return tempInfo.substring(7, tempInfo.indexOf("packet"));
//                } else {
...
//            }
    ...
    private static String ping(String command) {
        ...
        try {
            ...
            BufferedReader reader = new BufferedReader(new InputStreamReader(is));//内存溢出，后面换种写法
