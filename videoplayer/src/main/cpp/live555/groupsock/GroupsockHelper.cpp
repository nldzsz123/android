...
static int createSocket(int type) {
  ...
  int sock;
/**
 *  对于系统支持SOCK_CLOEXEC和FD_CLOEXEC标志的，必须在创建socket的时候进行设置，否则会引起不可预料的问题。
 */
#ifdef SOCK_CLOEXEC
...
//    printf("%s 开始读取==>%d\n",getTimeFormat(),bufferSize);
  int bytesRead = (int)recvfrom(socket, (char*)buffer, bufferSize, 0,
			   (struct sockaddr*)&fromAddress,
			   &addressSize);
//    printf("%s 读取结束==>%d\n",getTimeFormat(),bytesRead);
  if (bytesRead < 0) {      //1. 如果接收缓冲区没有数据，返回-1 系统错误为EAGAIN或者其它指定的非零的值 windows系统和linux系统不一样
    ...
    //##### END HACK
    socketErr(env, "recvfrom() error: ");   // 2. 如果出现错误 也返回-1 这两种情况根据errno来区分
  } else if (bytesRead == 0) {  //3. 如果远程socket关闭了，返回0；
    // "recvfrom()" on a stream socket can return 0 if the remote end has closed the connection.  Treat this as an error:
    return -1;
  }

  return bytesRead; // 读取的数据不会超过bufferSize的大小
}
