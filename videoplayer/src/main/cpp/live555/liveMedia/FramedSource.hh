...
public:
  ...
  // 用于回调的函数 包含回调对象 传回回调的参数。frameSize 帧大小，numTruncatedBytes 被截断的字节数
  ...
  // 别人回调自己的函数 clientData 对象是自己。当sourcce对象要关闭之前会调用一下
  ...
  // 别人回调自己的函数 clientData 对象是自己。当sourcce对象获取到一帧数据之后回调一次，他会调用doGetNextFrame()函数。
