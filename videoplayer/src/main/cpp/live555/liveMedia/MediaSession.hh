...
public:
  ...
  char*& _absStartTime() { return fAbsStartTime; }  // char*& 字符指针的引用
...
protected:
  ...
  Boolean parseSDPLine_s(char const* sdpLine);  // 为fSessionName赋值
  Boolean parseSDPLine_i(char const* sdpLine);  // 为fSessionDescription赋值
  Boolean parseSDPLine_c(char const* sdpLine);  // 为fConnectionEndpointName赋值 id地址信息
  Boolean parseSDPAttribute_type(char const* sdpLine);  // 为fMediaSessionType赋值
  Boolean parseSDPAttribute_control(char const* sdpLine);   // 为fSessionName赋值
  Boolean parseSDPAttribute_range(char const* sdpLine); // 为fControlPath赋值
  Boolean parseSDPAttribute_source_filter(char const* sdpLine); // 为fSourceFilterAddr赋值
