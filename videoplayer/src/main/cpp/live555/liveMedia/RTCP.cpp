...
#ifdef DDEBUG
    fprintf(stderr, "%s [%p]saw incoming RTCP packet (from ",getTimeFormat(), this);
    if (tcpSocketNum < 0) {
...
#ifdef DDEBUG
  fprintf(stderr, "%s sending REPORT\n",getTimeFormat());
#endif
...
#ifdef DDEBUG
  fprintf(stderr, "%s sending RTCP packet\n",getTimeFormat());
...
Boolean RTCPInstance::addReport(Boolean alwaysAdd) {
  ...
  if (fSink != NULL) {  // 表示发送端的报告 则是SR的报告
    if (!alwaysAdd) {   // 默认为false
    ...
    addSR();
  } else if (fSource != NULL) { // 表示接收端的曝光，RR报告
    if (!alwaysAdd) {
...
//    envir() << getTimeFormat() << "highestExtSeqNumReceived==>" << highestExtSeqNumReceived << "\n";
//    envir() << getTimeFormat() << "totNumExpected==>" << totNumExpected << "\n";
//    envir() << getTimeFormat() << "numExpectedSinceLastReset==>" << numExpectedSinceLastReset << "\n";
//    envir() << getTimeFormat() << "lossFraction==>" << lossFraction << "\n";
...
#ifdef DDEBUG
  fprintf(stderr, "%s schedule(%f->%f)\n", getTimeFormat(),secondsToDelay, nextTime);
#endif
