...
RTSPClient* RTSPClient::createNew(UsageEnvironment& env, char const* rtspURL,
  ...
  return new RTSPClient(env, rtspURL,
			verbosityLevel, applicationName, socketNumToServer);
}
...
unsigned RTSPClient::sendGetParameterCommand(MediaSession& session, responseHandler* responseHandler, char const* parameterName,
  ...
  size_t parameterNameLen = parameterName == NULL ? 0 : strlen(parameterName);
...
void RTSPClient::setUserAgentString(char const* userAgentName) {
  ...
  unsigned long const headerSize = strlen(formatStr) + strlen(userAgentName);
  delete[] fUserAgentHeaderStr;
  fUserAgentHeaderStr = new char[headerSize];
  sprintf(fUserAgentHeaderStr, formatStr, userAgentName);
  fUserAgentHeaderStrLen = (unsigned int)strlen(fUserAgentHeaderStr);
}
...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
  ...
  unsigned long userAgentNameSize
...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  char* cmd = NULL;
  do {
    // 第一次发送Describe消息时，先连接上socket，等到连接上之后，开启读取的消息监控，然后发送describe消息。
    ...
    } else if (fInputSocketNum < 0) { // we need to open a connection 说明还没有创建socket
    ...
    unsigned contentStrLen = (unsigned)strlen(contentStr);
    if (contentStrLen > 0) {
      char const* contentLengthHeaderFmt =
	"Content-Length: %d\r\n";
      unsigned contentLengthHeaderSize = (unsigned)strlen(contentLengthHeaderFmt)
	+ 20 /* max int len */;
    ...
    char const* const cmdFmt =
      ...
      "\r\n"
      "%s";
    unsigned long cmdSize = strlen(cmdFmt)
    ...
    // 在阻塞模式下，send函数将数据拷贝到发送缓存中，然后返回。如果发送缓存中空间不足，则一直等待直到有空间完全拷贝过去为止；在非阻塞模式下，如果空间不够用，则尽力的拷贝，返回拷贝成功的数据大小，如缓存区可用空间为0,则返回-1,同时设置errno为EAGAIN.
    if(send(fOutputSocketNum, cmd, strlen(cmd), 0) < 0) {
      char const* errFmt = "%s send() failed: ";
      unsigned long const errLength = strlen(errFmt) + strlen(request->commandName());
    ...
    // 请求发送成功后，将该请求在放入消息响应队列，等待消息有回应后再进行响应的回调
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
  ...
  // 对于tcp的三次握手就是在connect函数中实现的
  // 如果返回0，表示连接成功，这种情况一般在本机上连接时会出现(否则怎么可能那么快)
  // 否则，查看error是否等于EINPROGRESS或者EWOULDBLOCK(表明正在进行连接中)，如果不等于，则连接失败
  if (connect(socketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0) { // 非阻塞方式都是立即返回的
    int const err = envir().getErrno();
    if (err == EINPROGRESS || err == EWOULDBLOCK) { // 对于非阻塞试 都采用这种方式判断是否连接
...
char* RTSPClient::createAuthenticatorString(char const* cmd, char const* url) {
  ...
  if (auth.realm() != NULL && auth.username() != NULL && auth.password() != NULL) {
	...
	"nonce=\"%s\", uri=\"%s\", response=\"%s\"\r\n";
      ...
      unsigned long authBufSize = strlen(authFmt)
	...
	+ strlen(auth.nonce()) + strlen(url) + strlen(response);
      ...
      unsigned long usernamePasswordLength = strlen(auth.username()) + 1 + strlen(auth.password());
      char* usernamePassword = new char[usernamePasswordLength+1];
      sprintf(usernamePassword, "%s:%s", auth.username(), auth.password());

      char* response = base64Encode(usernamePassword, (unsigned)usernamePasswordLength);
      unsigned long const authBufSize = strlen(authFmt) + strlen(response) + 1;
      authenticatorStr = new char[authBufSize];
...
void RTSPClient::incomingDataHandler1() {
  ...
  // 它是非阻塞函数，立即返回的。-1 socket发生错误 0 socket的接收缓冲区没有数据 >0 正确读取到了数据
...
static char* getLine(char* startOfLine) {
    ...
    for (char* ptr = startOfLine; *ptr != '\0'; ++ptr) {
        ...
        // \r\n，\r，\n 都作为一行的结束 碰到这三种情况后自动将它们所处的位置替换为字符串的结束符'\0'。然后将该字符出现的位置的下一个位置作为开始。如果连续是\r\n\r\n 则startOfLine 将以'\0'开始
        if (*ptr == '\r' || *ptr == '\n') {
...
void RTSPClient::handleResponseBytes(int newBytesRead) {
    do {
        // data was read OK; process it below。读取的数据是正常的(包括socket缓冲区没有数据的情况)
        if (newBytesRead >= 0 && (unsigned)newBytesRead < fResponseBufferBytesLeft) break;
        
        // 读取的数据长度超过了接收缓冲区剩余的缓冲大小 则认为此次请求也失败。因为数据没有读取完全，所以要将接收回应的缓冲区设置的足够大
        ...
        // 走到这一步就说明socket读取发生了错误；分为两种情况 1、rtspClient的接收缓冲区过小 2、socket读取发生错误(socket关闭，读取错误等)
        ...
        resetResponseBuffer();
        RequestRecord* request;
        if (newBytesRead > 0) { // The "RTSP response was truncated" error 读取的数据长度超过了接收缓冲区剩余的缓冲长度
            if ((request = fRequestsAwaitingResponse.dequeue()) != NULL) {
                handleRequestError(request);
                delete request;
            }
        } else {    // socket发生了错误，也就是返回了-1
    ...
    unsigned numExtraBytesAfterResponse = 0;    // 解析完一个回应后接受缓冲区剩余的字节数
    Boolean responseSuccess = False; // by default
    do {    // 循环解析接受缓冲区中的数据，找出每一个响应。有可能不是一个完整的响应
        ...
        do {    // 最外层循环 循环找出接受缓冲区中的所有消息响应。
            headerDataCopy = new char[responseBufferSize];
            // 将fResponseBuffer中的前fResponseBytesAlreadySeen个字节复制到headerDataCopy中
            ...
            while (1) { // 找到一个消息响应的开头，则找出这个完整的消息
                ...
                nextLineStart = getLine(lineStart);
                if (lineStart[0] == '\0') {
                    break; // this is a blank line。只有连续出现 \r\n\r\n时才会走到这里，消息头和消息体之间会走这里
                }
                ...
                if (checkForHeader(lineStart, "CSeq:", 5, headerParamsStr)) {
                    ...
                    // Find the handler function for "cseq": 这里的seq是之前request的序列号
                    RequestRecord* request;
            ...
            unsigned bodyOffset = nextLineStart == NULL ? fResponseBytesAlreadySeen : (unsigned)(nextLineStart - headerDataCopy);
            bodyStart = &fResponseBuffer[bodyOffset];
            numBodyBytes = fResponseBytesAlreadySeen - bodyOffset;
            if (contentLength > numBodyBytes) { // 如果接受缓冲区没有形成一个完整的消息，则继续读取数据
                // We need to read more data.  First, make sure we have enough space for it: 消息体没有读取完整，需要在读取数据
                unsigned numExtraBytesNeeded = contentLength - numBodyBytes;    // 需要再额外读取的数据
            ...
            numExtraBytesAfterResponse = (unsigned)(&fResponseBuffer[fResponseBytesAlreadySeen] - responseEnd);
        ...
        // 前面处理完了一个消息，消息接收缓冲区中还剩余字节没有处理，则将这些剩余的字节移到缓冲区的开头进行下一个消息的解析
...
RTSPClient::RequestRecord* RTSPClient::RequestQueue::dequeue() {
  ...
  if (fHead == fTail) {     // 说明该链表只有一个节点了。
    fHead = NULL;
    fTail = NULL;
  } else {
    fHead = fHead->next();  // 否则还有多个节点，那么把head往后移
  }
  if (request != NULL) request->next() = NULL;  // 由于只需要返回一个节点，所以要将它的next赋值为null
  return request;
}
