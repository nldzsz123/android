...
/**
 *  sdp的正确格式为：
 *  sdp会话描述由多行<type>=<value>组成，其中<type>是字符，<value>是一个字符串，其格式视<type>而定，整个sdp格式协议区分大小写，
 *  "="两侧不允许有空格；由会话级描述和媒体级描述组成，其中会话级描述从v=开始，至第一个m=结束，会话级描述的参数作用域是全局的，媒体级描述从
 *  m=开始至下一个m=结束，参数的作用域在该媒体范围内。会话级描述只能有一个，媒体级描述可以至少有一个，比如一个视频流和一个音频流。
 
    v=0     版本信息
    o=- 1514764884433479 1 IN IP4 192.168.42.1  <用户名> <session id> <会话版本> <网络类型><地址类型> <地址>。<用户名>用户的登录名，如果服务器不支持则为-；<session id> 数字串，唯一，建议用NTP timestamp；
    s=Ambarella streaming   会话名
    i=Ambarella streaming   会话信息
    t=0 0 <开始时间> <结束时间> <结束时间>为零表示过了<开始时间>后会话一直持续，当<开始时间> 和<结束时间>均为零时表示持久会话。
    ...
    m=video 0 RTP/AVP 96    <流类型> <传输层端口号> <传输方式> <playload type>。表示视频流 用udp方式传输视频帧 视频流play type都是96
    c=IN IP4 0.0.0.0
    b=AS:10000  建议的带宽 单位kb/s
    a=rtpmap:96 H264/90000
    a=fmtp:96 packetization-mode=1;profile-level-id=4D002A;sprop-parameter-sets=J00AKppkAoAt01AQEBQAAA+kAAOpg6GACAAAB//rvLjQwAQAAAMD//XeXCg=,KO48gA==
    a=control:track1
 */
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
    ...
    char const* sdpLine = sdpDescription;   //上一行
    char const* nextSDPLine;    // 找出下一行参数
    while (1) { // 将会话级描述解析出来 v=开始至第一个m=结束
        if (!parseSDPLine(sdpLine, nextSDPLine)) return False;  //从开始取出一行，并分析这一行是否正确的sdp语法格式
        //##### We should really check for the correct SDP version (v=0)
        if (sdpLine[0] == 'm') break;   //碰到第一个m=跳出解析
        sdpLine = nextSDPLine;
    ...
    while (sdpLine != NULL) {   // 开始解析媒体级描述从第一个m=开始至下一个m=结束
        ...
        if ((sscanf(sdpLine, "m=%s %hu RTP/AVP %u",mediumName, &subsession->fClientPortNum, &payloadFormat) == 3 ||sscanf(sdpLine, "m=%s %hu/%*u RTP/AVP %u",mediumName, &subsession->fClientPortNum, &payloadFormat) == 3) && payloadFormat <= 127) {
            
            protocolName = "RTP";
            
        } else if ((sscanf(sdpLine, "m=%s %hu UDP %u",mediumName, &subsession->fClientPortNum, &payloadFormat) == 3 ||sscanf(sdpLine, "m=%s %hu udp %u",mediumName, &subsession->fClientPortNum, &payloadFormat) == 3 ||
            sscanf(sdpLine, "m=%s %hu RAW/RAW/UDP %u",mediumName, &subsession->fClientPortNum, &payloadFormat) == 3) && payloadFormat <= 127) {
...
/**
 *  const char* 和char const * 一样，指向字符串常亮，不可以通过指针改变字符串内容。
 *  char* const 指针常亮，可以通过指针改变字符串内容，但是无法改变指针的指向
 */
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
    ...
    do {
        if (fCodecName == NULL) {   // 流媒体的编码方式 比如H264
        ...
        if (fClientPortNum != 0 && (honorSDPPortChoice || IsMulticastAddress(tempAddr.s_addr))) {
            ...
            while (1) {
                ...
                // To be usable for RTP, the client port number must be even(奇数):
                if ((fClientPortNum&1) != 0) { // it's odd  偶数
...
Boolean MediaSubsession::createSourceObjects(int useSpecialRTPoffset) {
  do {
      ...
      if (strcmp(fCodecName, "H264") == 0) {
          ...
          fReadSource = fRTPSource = H265VideoRTPSource::createNew(env(), fRTPSocket,fRTPPayloadFormat,expectDONFields,fRTPTimestampFrequency);
      } else {
