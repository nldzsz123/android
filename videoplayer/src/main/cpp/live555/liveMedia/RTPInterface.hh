...
// 用于类的回调 相当于OC的Delegate。当从socket读取到数据时，如果设置了回调函数，则调用一次。
...
// 是一个链表 新创建的节点将加入next的前面，作为Head节点；包含socket文件描述句柄和streamId字符串，对象是根据streamId来区分不同的流的，流可能共用一个socket。
...
/**
 *  它有一个 变量 Groupsock 用来收发UDP数据包，它还有一个变量 Medium 用于收发数据包之后的回调处理
 */
class RTPInterface {
public:
